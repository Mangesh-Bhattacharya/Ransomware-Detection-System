...
def start_gui():
    ...
    tk.Button(button_frame, text="Start System Scan", command=lambda: start_scan(log_widget), bg="#4dff4d",
              fg="black", font=("Arial", 10, "bold"), width=15).grid(row=0, column=0, padx=5, pady=5)
    tk.Button(button_frame, text="Force Kill Malware", command=block_ransomware, bg="#ff4d4d",
              fg="white", font=("Arial", 10, "bold"), width=15).grid(row=0, column=1, padx=5, pady=5)
    tk.Button(button_frame, text="View Log", command=lambda: os.system(f'notepad.exe logs/ransomware_scan_log.csv'),
